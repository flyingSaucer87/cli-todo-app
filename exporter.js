...
function escapeCsvCell(v) {
  ...
  const s = String(v);
  // double quotes inside quotes must be doubled per CSV rules
  return `"${s.replace(/"/g, '""')}"`;
}
...
function exportToICS(outPath) {
  ...
  const header = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//cli-todo-app//EN'
  ];
  const footer = ['END:VCALENDAR'];
  const events = [];
  todos.forEach((t, i) => {
    // ICS export relies on a due date. If your todos.json items don't have 'due',
    // they won't become events. You can adapt by using created_at or skipping.
    if (!t.due) return;
  ...
  fs.writeFileSync(outPath, header.concat(events).concat(footer).join('\r\n'), 'utf8');
  console.log(`Exported ${events.length / 6 | 0} events → ${outPath} (tasks with due dates only)`);
}
...
function importFromCSV(inPath) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    items.push(normalize({
      ...
      completed: (obj.completed === 'true' || obj.completed === '1' || String(obj.completed).toLowerCase() === 'x'),
...
// --- public API ---
function exportTodos(format, outPath) {
...
function importTodos(format, inPath) {
  format = (format || 'csv').toLowerCase();
  if (!inPath) {
