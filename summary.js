import { addTodo, listTodos, removeTodo, editTodo, completeTodo, clearTodos, getSummary } from "./todo.js";
...
Usage:
  ...
  node index.js summary [day|week] - Display a summary of tasks
...
switch (command) {
  ...
  case "clear":
    ...
    case "summary":
        const period = parsed.positional[0] || 'day';
        if (period !== 'day' && period !== 'week') {
            console.log("Error: Invalid period. Please use 'day' or 'week'.");
            process.exit(1);
        }
        const summary = getSummary(period);
        console.log(chalk.bold.hex(theme.accent)(`Task Summary for the ${period === 'day' ? 'Day' : 'Week'}`));
